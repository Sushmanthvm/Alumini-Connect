...
import { ArrowLeft, Award, Briefcase, Code2, Mail, MapPin, Sparkles, Trophy } from "lucide-react";
...
function AlumniDetail() {
  ...
  const sendRequest = (e: React.FormEvent) => {
    ...
    toast.success(`${intent} request sent to ${alum.name}!`, {
      description: `Subject: "${subject}" · Resume: ${resume?.name ?? "none"} · Reply at ${email}.`,
    });
  };
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[1fr_2fr]">
          ...
          >
            <div className="mx-auto h-32 w-32 overflow-hidden rounded-3xl ring-4 ring-white shadow-elegant">
              <img src={alum.photo} alt={alum.name} className="h-full w-full" />
            </div>
            ...
            <p className="text-center text-sm text-muted-foreground">{alum.company} · Batch of {alum.batch}</p>
          ...
          <div className="space-y-6">
            ...
            <Section icon={<Code2 className="h-4 w-4" />} title="Tech Used">
              <div className="flex flex-wrap gap-2">
                {alum.tech.map((t) => (
                  <span key={t} className="rounded-full border border-border bg-card px-3 py-1 text-sm">{t}</span>
                ))}
              </div>
            </Section>
            ...
            <Section icon={<Trophy className="h-4 w-4" />} title="Extra Curricular" delay={0.4}>
              <ul className="space-y-2">
                {[
                  ...
                  "National Hackathon Organizer",
                  "Open-Source Contributor (500+ stars)",
                  "Volunteer Mentor at CodeForChange",
                ].map((c) => (
                  <li key={c} className="flex items-center gap-2 text-sm">
          ...
          <DialogContent>
            ...
            <form onSubmit={sendRequest} className="space-y-4 pt-2">
              ...
              <DialogFooter>
                ...
                <Button type="submit" className="gradient-bg-hero text-primary-foreground">Send Request</Button>
              </DialogFooter>
            </form>
          </DialogContent>
        </Dialog>
